...
H-Mode7 is a CPU software renderer for RPG Maker XP (RGSS1). It projects a 2D tile map into a pseudo-3D Mode-7-style view with per-tile height, animated autotiles, vertical wall textures, dynamic lighting, shadows, and billboard-sprite "surfaces" with opacity and blend modes. It runs as a per-pixel, per-scanline software rasterizer that reads and writes RGSS `Bitmap` and `Table` memory directly. No GPU, no shaders.

The DLL is self-contained. Its only Win32 surface:

- `<windows.h>` typedefs (`DWORD`, `LPBYTE`, `RGBQUAD`, `BITMAPINFOHEADER`). Replace with `uint32_t`, `uint8_t*`, and equivalents.
- `__declspec(dllexport)` / `__declspec(dllimport)`. Remove.
- `Win32API.new` on the Ruby side. Replace with `rb_define_module_function`.

No GDI, no DirectX, no threading, no IO, no CRT beyond `malloc`/`free`. Math is integer-only (the included `<math.h>` is unused). Most of the porting work lives in two places: replacing the RGSS1 ABI handling with modern MRI equivalents, and adapting the Bitmap pixel layout to mkxp-z's SDL/GL-backed `Bitmap`. The same source compiles against Ruby 1.8 through 3.1 in mkxp-z-apple-mobile's per-version pipeline. One ABI quirk surfaces: Ruby 1.9's strict C++ callback signature on `rb_hash_foreach`, fixed with an `(int (*)(ANYARGS))` cast at the call site.

Porting challenges, in order of effort:

1. **Bitmap pixel access.** RGSS1 hands C code a flat pointer to a bottom-up DIB. mkxp-z's `Bitmap` wraps a GL texture plus an `SDL_Surface`. The port needs a `raw_data` / `rawData`-style accessor that returns linear CPU-visible memory in BGRA order, and a hand-back path to sync the GPU texture once the renderer finishes writing.
2. **Table ABI.** RGSS1 exposed `Table` with `SWORD* data` at a known offset. mkxp-z's C++ `Table` exposes the buffer through `Table::at(x, y, z)` and a data pointer.
3. **Hash iteration.** The original DLL walked `st_table` bin chains directly. Modern Ruby (1.9+) made `st_table` opaque and switched to open addressing in 2.6+. The port iterates with `rb_hash_foreach`. Ruby 1.9's strict C++ checking needs an `(int (*)(ANYARGS))` cast on the callback for the same source to compile under 1.9.
4. **Integer `__id__` tagging.** RGSS1 turned `id << 1` into a raw `VALUE` pointer. Modern Ruby returns a Fixnum ID that isn't the raw VALUE. The port skips `__id__` entirely and binds with the standard `VALUE self, VALUE arg1, ...` shape via `rb_define_method`.
5. **The `renderHM7` kernel** (lines 763–1767 of the reference, around 1010 LOC). One huge function with nested branches. Splitting it into helpers without changing behavior dominates the port effort.

---

## 1. Algorithm Summary (Per Exported Function)

Notation: "BGRA" means a 4-byte per pixel layout `{B, G, R, A}` in memory order (DIB native on Windows, identical to `SDL_PIXELFORMAT_ARGB8888` on little-endian or `SDL_PIXELFORMAT_BGRA32` depending on SDL version: see §3).
...
`colormap` ends up as a wide bitmap (`width = 5*32 = 160`, height proportional to tile count) that subsequent `renderHM7` walls sample vertically via `colormapData = colormapBegin - ((tileIndex*10) << 6) + …`.
...
**What:** The rasterizer. For each screen row top-to-bottom (reading bottom-to-top** because heights grow upwards), for each screen column (with 1×/2× horizontal filter step), it:
...
- **Main descending loop yt = yMax-1..y0:** cache `lux_*` and `h_coeff` from `lightline` for this row.
  - **PRE-SURFACES pass (lines 960–1201):** only on the first row, draws any surfaces whose `screenY1` is below the first row, projecting them up to their `sHinit` top. Writes into `sScreenBitmap` scratch.
...
| 2   | altitude      | `distance_h` (camera distance): NOTE: takes `>> 1` on tag strip twice → `distance_h`, not fixed-point | tagged int |
...
Note: indices 0, 1, 4, 5, 7, 8 are **not** tagged (they are scaled into integer space on the Ruby side and passed without `<<1`-tagging: the C code reads them with raw `*ptr` without `>>1`). This is a subtle ABI detail: some entries are Ruby Fixnums (needing `>>1`), some are raw DWORDs masquerading as VALUEs: the Ruby side packs them all as Fixnums (positive values, so high bit 0, and the tag bit is bit 0), but the C side is sloppy about which ones to untag. **For our port, pass them as regular `int` arguments or an `RArray` of `FIX2INT`-extracted values without any `<<1`/`>>1` voodoo.**
...
- Per-column perspective correction derived from `sCmin/sCmax/sCsl`: these come from `lightline` row-1 horizontal-zoom coefficients, giving a perspective projection (not linear interpolation). The formula:
  ```
...
`renderHM7` does four `malloc`s of `nbLayers * sizeof(X)` bytes and frees them at the end. With `nbLayers==3` that's 4 tiny allocations per frame: negligible but **improved by moving them to the stack** (`char initA[MAX_LAYERS=8]`) or a single persistent scratch buffer held in the plugin struct.
...
- **Types only** from `<windows.h>`: `DWORD`, `LPBYTE`, `RGBQUAD`, `BITMAPINFOHEADER`. **No function calls.** Replace with `uint32_t`, `uint8_t*`, a `struct { uint8_t b,g,r,a; }`, and nothing (the port doesn't need `BITMAPINFOHEADER`; width/height/pitch come from the engine's Bitmap wrapper).
