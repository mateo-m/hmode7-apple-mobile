...
H-Mode7 is a pseudo-3D rotating-perspective renderer. It adds slanted top-down maps with zoom, rotation, and heightmap-based terrain elevation to RPG Maker XP, and is used by various RPG Maker XP fan games for cutscenes and overworld effects.
...
- **Multi-Ruby compatible.** The same source compiles against Ruby 1.8, 1.9, 3.0, and 3.1 in mkxp-z-apple-mobile's per-version build pipeline, so RGSS1 games on Ruby 1.8 and modern forks on Ruby 3.1 both get hmode7 dispatched via the matching merged `.o`.
...
- **Reference-matching output.** The full feature set renders matching the Windows reference: wall textures, Z-ordering, billboard sprites, horizon fading, heightmap relief, autotile animation, and multi-layer tile-facade rendering for buildings.
...
Complete and verified against the public V1.4.4 source. All nine H-Mode7 exports are covered, for both DLL eras:
...
Adapting to other mkxp-z-based engines requires minor build-system changes. The `cpp` files compile against the engine's binding layer; nothing platform-specific.
